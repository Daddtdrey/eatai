...
import { 
  ...
  ShieldCheck, Edit, Info, ChevronDown, ChevronUp, Dumbbell, Store, LogIn, Filter, Bell, Bike, History
} from 'lucide-react';
import { 
  ...
  getAllOrders, updateOrderStatus, updateProduct, db, collection, onSnapshot, query, where, orderBy
} from './firebase'; 
...
// --- 🔒 CONFIGURATION 🔒 ---
const PAYSTACK_KEY = "pk_live_e26a023051d0eb34273cc6f86ccbf0e26ebbfdb9"; // REPLACE THIS

const SUPER_ADMINS = ["mannikdaniel@gmail.com"]; 
...
const SUB_ADMINS = {
    ...
    "coop.guy@gmail.com": "COOPERATIVE"
};

const LOCATIONS = ["Irrua", "Ekpoma", "Uromi"];
const VENDORS_BY_LOCATION = {
    "Irrua": ["NASCO", "NAISHAT", "COOPERATIVE", "Mama Irrua"],
    "Ekpoma": ["Chicken Republic", "Mama Ebo", "Campus Fit"],
...
const LogisticsView = ({ setCurrentView, user, setNotification }) => {
    ...
    useEffect(() => {
        // Fetch confirmed, picked_up, AND delivered orders to allow history view
                ...
                <div className="flex-1 overflow-y-auto pb-24 scrollbar-hide space-y-4">
                    ...
                    {displayedTasks.map(task => (
                        <div key={task.id} className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                            ...
                            <div className="bg-gray-50 dark:bg-gray-900/50 p-3 rounded-lg mb-3 text-sm">
                                <p><strong>To:</strong> {task.deliveryAddress}</p>
                                <p><strong>Contact:</strong> {task.phone}</p>
                                <p className="text-xs text-gray-500 mt-1">{task.items.length} Items | Fee: ₦{task.deliveryFee}</p>
                            </div>
...
const AdminView = ({ setCurrentView, marketData, refreshData, user, setNotification }) => {
  ...
  const [newItem, setNewItem] = useState({ name: '', price: '', category: 'fullMeal', desc: '', stock: 10, image: '', location: 'Irrua', vendor: isSuperAdmin ? '' : myVendorName });
  ...
  const handleCancelEdit = () => { setIsEditing(false); setEditId(null); setNewItem({ name: '', price: '', vendor: isSuperAdmin ? '' : myVendorName, category: 'fullMeal', desc: '', stock: 10, image: '', location: 'Irrua' }); };
  const handleSubmit = async (e) => { e.preventDefault(); setIsSubmitting(true); let imageIcon = newItem.image || '🍽️'; const finalVendor = isSuperAdmin ? newItem.vendor : myVendorName; const productPayload = { ...newItem, vendor: finalVendor, price: parseFloat(newItem.price), stock: parseInt(newItem.stock), image: imageIcon }; if (isEditing) { await updateProduct(editId, productPayload); alert("Updated!"); } else { await addProduct(productPayload); alert("Added!"); } handleCancelEdit(); await refreshData(); setIsSubmitting(false); };
  ...
  return (
    <ViewContainer title="Manager HQ" showBack onBack={() => setCurrentView('home')}>
      <div className="mb-4 p-3 bg-orange-50 dark:bg-gray-700 rounded-lg"><p className="text-xs font-bold text-gray-500 uppercase">Logged in as</p><p className="font-bold text-orange-600">{isSuperAdmin ? "SUPER ADMIN" : `${myVendorName} ADMIN`}</p></div>
      ...
      <div className="flex-1 overflow-y-auto pb-32 scrollbar-hide">
        ...
        {activeTab === 'products' && 
            <>
            <div id="admin-form" className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 mb-8"><div className="flex justify-between items-center mb-4"><h3 className="font-bold text-gray-800 dark:text-white">{isEditing ? 'Edit Item' : 'Add Item'}</h3>{isEditing && <button onClick={handleCancelEdit} className="text-xs text-red-500">Cancel</button>}</div><form onSubmit={handleSubmit} className="space-y-4"><div className="grid grid-cols-2 gap-4"><input required placeholder="Name" className="p-3 rounded-xl bg-gray-50 dark:bg-gray-700 border-none w-full dark:text-white" value={newItem.name} onChange={e => setNewItem({...newItem, name: e.target.value})} /><input required type="number" placeholder="Price (₦)" className="p-3 rounded-xl bg-gray-50 dark:bg-gray-700 border-none w-full dark:text-white" value={newItem.price} onChange={e => setNewItem({...newItem, price: e.target.value})} /></div><div className="grid grid-cols-2 gap-4"><input required placeholder="Vendor" disabled={!isSuperAdmin} className={`p-3 rounded-xl border-none w-full dark:text-white ${!isSuperAdmin ? 'bg-gray-200 dark:bg-gray-600' : 'bg-gray-50 dark:bg-gray-700'}`} value={newItem.vendor} onChange={e => setNewItem({...newItem, vendor: e.target.value})} /><select className="p-3 rounded-xl bg-gray-50 dark:bg-gray-700 border-none w-full dark:text-white" value={newItem.category} onChange={e => setNewItem({...newItem, category: e.target.value})}><option value="fullMeal">Meal</option><option value="fitness">Fitness</option><option value="pregnancy">Pregnancy</option><option value="period">Period</option><option value="male">Male</option><option value="normal">Normal</option></select></div><div className="bg-gray-100 dark:bg-gray-700 p-3 rounded-xl"><label className="text-xs text-gray-500 mb-1 block uppercase">Location</label><div className="flex gap-2">{LOCATIONS.map(loc => (<button type="button" key={loc} onClick={() => setNewItem({...newItem, location: loc})} className={`flex-1 py-1 rounded text-xs font-bold ${newItem.location === loc ? 'bg-orange-500 text-white' : 'bg-gray-200 text-gray-600'}`}>{loc}</button>))}</div></div><div className="grid grid-cols-2 gap-4"><div className="flex items-center gap-2 bg-gray-50 dark:bg-gray-700 rounded-xl p-3"><Box className="w-5 h-5 text-gray-500" /><input required type="number" placeholder="Stock" className="bg-transparent border-none w-full outline-none dark:text-white" value={newItem.stock} onChange={e => setNewItem({...newItem, stock: e.target.value})} /></div><div className="flex items-center gap-2 bg-gray-50 dark:bg-gray-700 rounded-xl p-3"><Smile className="w-5 h-5 text-gray-500" /><input placeholder="Emoji" className="bg-transparent border-none w-full outline-none dark:text-white" value={newItem.image} onChange={e => setNewItem({...newItem, image: e.target.value})} /></div></div><textarea required placeholder="Description..." className="w-full p-3 rounded-xl bg-gray-50 dark:bg-gray-700 border-none h-20 resize-none dark:text-white" value={newItem.desc} onChange={e => setNewItem({...newItem, desc: e.target.value})} /><button disabled={isSubmitting} className="w-full bg-gray-900 dark:bg-orange-600 text-white font-bold py-3 rounded-xl">{isSubmitting ? 'Saving...' : 'Save Item'}</button></form></div><div className="space-y-3">{filteredMarketData.map(item => (<ProductCard key={item.id} item={item} isAdmin={true} onEdit={handleEditClick} onDelete={handleDelete} />))}</div></>}
      </div>
    </ViewContainer>
  );
};
...
const PaymentModal = ({ isOpen, onClose, total, paymentMethod, user, cart, globalWallet, onSuccess, city }) => {
  ...
  const handlePayment = async (method = paymentMethod) => {
    ...
    if (method === 'transfer' && !form.transferName) return alert("Sender name required");
    setProcessing(true);
    if (method !== 'paystack') await new Promise(r => setTimeout(r, 1500));
    try {
        await createOrder(user.uid, cart, grandTotal, method, globalWallet?.address, form.address, form.transferName || "Paystack", form.phone, form.landmark, deliveryFee, method === 'paystack' ? 'confirmed' : 'pending');
    ...
    <div className={darkMode ? "dark" : ""}>
      <div className="bg-gray-50 dark:bg-gray-950 min-h-screen font-sans text-gray-900 dark:text-white relative overflow-hidden">
        ...
        <main className="h-[calc(100vh-140px)] md:h-[calc(100vh-72px)] overflow-hidden">
            ...
            {currentView === 'vendors' && <VendorSelectionView city={city} setVendor={setVendor} setCurrentView={setCurrentView} />}
