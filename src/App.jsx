...
export default function EatAi() {
    ...
    // --- HANDLERS ---
    // Helper: Calculate total price of a cart item including addons
    ...
    const addToCart = (item) => {
        ...
        let addonMsg = '';
        if (item.selectedAddons && item.selectedAddons.length > 0) {
            addonMsg = ` with ${item.selectedAddons.map(a => a.name).join(', ')}`;
        }

        setNotification(`Added ${qty}x ${item.name}${addonMsg} to cart!`);
    ...
    const cartTotal = cart.reduce((sum, item) => sum + calculateItemPrice(item), 0).toFixed(2);
