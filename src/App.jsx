      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1">
             ...
             <span className="text-[10px] border border-orange-200 text-orange-600 px-1.5 rounded">{item.location || 'HQ'}</span>
          </div>
          <div className="flex items-center gap-2 mt-1">
    ...
    <ViewContainer title="Select Location" showBack onBack={() => setCurrentView('home')}>
        <div className="grid grid-cols-1 gap-4 mt-4">
            {LOCATIONS.map((loc) => (
                <button 
                    key={loc}
                    onClick={() => { setCity(loc); setCurrentView('market'); }}
                    className="p-6 rounded-2xl bg-gradient-to-r from-orange-50 to-orange-100 dark:from-gray-800 dark:to-gray-700 flex items-center justify-between hover:scale-[1.02] transition-transform shadow-sm"
                >
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-white dark:bg-gray-600 rounded-full flex items-center justify-center text-2xl">
                            📍
                        </div>
    ...
    <div className="flex flex-col items-center justify-center h-full space-y-8 animate-fade-in p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-md">
        ...
        >
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-2 relative z-10">Generate recipes.</p>
        </button>
        <button 
          onClick={() => setCurrentView('location')} // Updated to go to Location first
        ...
        >
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-2 relative z-10">Meals & cravings.</p>
        </button>
      </div>
    </div>
);
    ...
    <ViewContainer title="AI Fridge Raider" showBack onBack={() => setCurrentView('home')}>
      <div className="flex-1 overflow-y-auto pb-24 scrollbar-hide">
        <div className="bg-orange-50 dark:bg-gray-800 p-6 rounded-3xl mb-6 border border-orange-100 dark:border-gray-700 transition-colors">
          ...
          >
            {isThinking ? <><Sparkles className="w-5 h-5 animate-spin" /><span>Thinking...</span></> : <><Sparkles className="w-5 h-5" /><span>Invent Recipe</span></>}
          </button>
        </div>
        {aiRecipe && (
...
const MarketView = ({ marketSection, setMarketSection, cravingsType, setCravingsType, setCurrentView, addToCart, marketData, loadingData, city }) => {
    ...
    if (!marketSection) {
      return (
        <ViewContainer title="The Market" showBack onBack={() => setCurrentView('location')}>
          <div className="space-y-4">
            ...
            >
              <div className="relative z-10 text-white"><h3 className="text-3xl font-black">Full Meals</h3><p className="opacity-90">Hearty dinners</p></div><span className="absolute bottom-2 right-2 text-6xl opacity-20">🍱</span>
            </button>
            <button 
              onClick={() => { setMarketSection('cravings'); setCravingsType(null); }}
            ...
            >
              <div className="relative z-10 text-white"><h3 className="text-3xl font-black">Cravings</h3><p className="opacity-90">For specific moments</p></div><span className="absolute bottom-2 right-2 text-6xl opacity-20">🍫</span>
            </button>
          </div>
        </ViewContainer>
      );
        ...
        {activeTab === 'orders' && (
            <div className="space-y-4">{adminOrders.map(order => (
                <div key={order.id} className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm">
                    ...
                    {order.status === 'pending' && (<button onClick={() => handleStatusUpdate(order.id, 'confirmed')} className="w-full bg-green-600 text-white py-3 rounded-lg text-sm font-bold shadow hover:bg-green-700">Confirm Payment & Order</button>)}
  ...
  const createWallet = async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        await saveWalletToProfile(user.uid, w.address); setWallet(walletData); setGlobalWallet(walletData);
  ...
  return (
    <div className={darkMode ? "dark" : ""}>
      <div className="bg-gray-50 dark:bg-gray-950 min-h-screen font-sans text-gray-900 dark:text-white relative overflow-hidden">
        ...
        <header className="hidden md:flex items-center justify-between px-8 py-4 bg-white dark:bg-gray-900 shadow-sm sticky top-0 z-40">
        ...
        <main className="h-[calc(100vh-140px)] md:h-[calc(100vh-72px)] overflow-hidden">
            ...
            {currentView === 'admin' && isAdmin && <AdminView setCurrentView={setCurrentView} marketData={marketData} refreshData={async () => setMarketData(await getAllProducts())} />}
            ...
            {currentView === 'market' && <MarketView marketSection={marketSection} setMarketSection={setMarketSection} cravingsType={cravingsType} setCravingsType={setCravingsType} setCurrentView={setCurrentView} addToCart={addToCart} marketData={marketData} loadingData={loadingData} city={city} />}
        </main>
        ...
        <nav className="fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800 px-6 py-4 flex justify-around items-center md:hidden z-40 safe-area-pb">
